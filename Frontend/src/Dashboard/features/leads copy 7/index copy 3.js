...
function InvestorList() {
   ...
   const fetchBlogData = async () => {
      try {
         const response = await api.get("/investor/get");
         const raw = response.data;
         const investors = Array.isArray(raw) ? raw : raw?.data || [];
         setBlogData(investors);
      } catch (err) {
   ...
   const handleDelete = async (id) => {
      ...
      try {
         await api.delete(`/investor/delete/${id}`);
   ...
   const handleCreateOrUpdate = async () => {
      try {
         if (isEditMode) {
            await api.put(`/investor/update/${editId}`, formData);
         } else {
            await api.post("/investor/createInvestor", formData);
         }
   ...
   return (
      <div className="p-6 min-h-screen bg-gray-100">
         <TitleCard title="Investor Relations Management">
            <div className="mb-6">
               <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
               {blogData.map((blog) => (
                  <div
                     key={blog._id}
                     className="border rounded-lg p-5 shadow-lg bg-white hover:shadow-xl transition-all flex flex-col justify-between"
                  >
                     <div>
                        {blog.image && (
                           <img
                        ...
                        <p className="text-sm font-medium text-orange-600 mt-1">{blog.subtitle}</p>
