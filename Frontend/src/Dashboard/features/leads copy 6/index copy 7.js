...
function JoinChef() {
   ...
   const fetchBlogData = async () => {
      try {
         const response = await api.get("/join/get");
         setBlogData(Array.isArray(response.data) ? response.data : [response.data]);
      } catch (err) {
         console.error("Failed to fetch join chef data", err);
      }
   };

   const handleDelete = async (blogId) => {
      try {
         await api.delete(`/join/delete/${blogId}`);
         setBlogData(blogData.filter((blog) => blog._id !== blogId));
      } catch (err) {
         console.error("Failed to delete join chef post", err);
      }
   };
   ...
   const handleCreateOrUpdate = async () => {
      try {
         if (isEditMode) {
            await api.put(`/join/update/${editId}`, formData);
         } else {
            await api.post("/join/createJoin", formData);
         }
         setIsCreateModalOpen(false);
         resetFormData();
         fetchBlogData();
      } catch (err) {
         console.error("Failed to submit join chef post", err);
      }
   };
   ...
   return (
      <div className="p-6 min-h-screen bg-gray-100">
         <TitleCard title="Join Chef Applications & Content">
            <div className="mb-6">
               <button
